...
export default function Map() {
  ...
  const handleFavoriteClick = async (id) => {
    const username = localStorage.getItem("username");
    try {
      const response = await fetch("http://localhost:4001/favorite", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    <div className="map-container">
      <h1 className="map-title" style={{ color: lightMode ? "black" : "white" }}>
        Interactive Map
      </h1>
      <GoogleMap
        center={center}
      ...
      >
        {shownLocations.map((location) => (
          <Marker
            ...
            onClick={() => handleMarkerClick({ latitude: location.latitude, longitude: location.longitude })}
          />
        ))}
            ...
            <div className="info-window">
              {selectedLocation.map((location) => (
                <div key={location.name}>
                  <h3 onClick={() => handleExactLocationClick(location)} className="location-name">
                    {location.name}
                  </h3>
        ...
        {exactLocation && (
          <div>
            ...
            <div className="exact-location-panel">
              ...
              <button className="favorite-button" onClick={() => handleFavoriteClick(exactLocation.id)}>
                Add favorite
              </button>
              <h2 className="location-title">{exactLocation.name}</h2>
              <p className="events-count">Events Count: {exactLocation.count}</p>
