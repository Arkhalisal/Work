...
export default function Events() {
  ...
  useEffect(() => {
    const tenLocationData = locationData.filter((location) => location.count > 3 && location.latitude).slice(0, 10);
  ...
  const handleLike = async (id) => {
    ...
    try {
      const response = await fetch("http://localhost:4001/like", {
        method: "POST",
        headers: {
      ...
      setLikeStatus((prevStatus) => ({ ...prevStatus, [id]: { liked: true, count: 1 } }));
      console.log(data);
    } catch (error) {
