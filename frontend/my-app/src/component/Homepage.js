...
function Homepage({ user, setUser }) {
  ...
  useEffect(() => {
    async function fetchEventData() {
      const response = await fetch("http://localhost:4001/getEventInfo");
      const data = await response.json();

      const venueResponse = await fetch("http://localhost:4001/getEventVenueInfo");
...
const Home = ({ user }) => {
  ...
  useEffect(() => {
    async function fetchBookedEvents() {
      const response = await fetch("http://localhost:4001/getBookedEvents", {
        method: "POST",
        headers: {
  ...
  const handleUnbookEvent = async (eventId) => {
    try {
      await fetch("http://localhost:4001/unbookEvent", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
