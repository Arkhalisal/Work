...
app.post("/login", async (req, res) => {
  try {
    ...
    const accessToken = jwt.sign(
      { username: user.username, isAdmin: user.isAdmin, password: hashedPassword },
      "secret"
    );
    ...
    if (unhashedPassword) {
      return res.json({ username: user.username, isAdmin: user.isAdmin, token: accessToken });
    } else {
...
app.post("/updateUser", async (req, res) => {
  try {
    ...
    await User.findOneAndUpdate({ username: oldUsername }, { username: newUsername, password: hashedPassword });
...
app.post("/updateLocation", async (req, res) => {
  try {
    ...
    await Venue.findOneAndUpdate({ id: id }, { comment: [...venue.comment, { username: username, comment: comment }] });
...
app.post("/favorite", async (req, res) => {
  try {
    ...
    await User.findOneAndUpdate({ username: username }, { favorites: [...user.favorites, id] });
    if (!user) {
      return res.json({ message: "User does not exist" });
    }
...
app.post("/booking", async (req, res) => {
  try {
    ...
    await User.findOneAndUpdate({ username: username }, { bookedEvents: [...user.bookedEvents, id] });
